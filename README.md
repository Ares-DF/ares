...
<img width="4000" height="3000" alt="Image" src="https://github.com/user-attachments/assets/d2eb6fa0-2248-4888-8397-642464d58f8e" />

**Ares is an air-gappable RF propagation simulation, geolocation, and passive-observation mission planning and execution platform built with Claude Code.** 
...
- **Bundled live DF ** — no external DF daemon: a registry driver pulls coherent multi-channel IQ straight off the radio (KrakenSDR / ANTSDR coherent chains, a clocked USRP set, a PlutoSDR with the 2R2T MIMO firmware, or a synthetic array for offline demo) and the DoA solver runs in-process, streaming bearings + fixes to the maps, the Emitter Summary, and ATAK as they're computed. Multi-VFO (several narrowband channels carved from one wideband capture, each squelched and DF'd independently), coherence auto-calibration when the front-end has a switchable reference, an antenna catalogue (ALARIS-class DF heads, KrakenSDR UCA, USRP ULA, …) that sets the array geometry + recommended method for you, GPS-tracked device positions, and a real spectrum/waterfall from the live radio in the DF panel. (External DF pipelines — krakensdr_doa, an Epiq-side process — are still supported as a "device" that streams pre-computed bearings in.)
- **Single-channel DF** — when you only have one SDR plus motion: RSS log-distance ML, RSS-gradient bearing, Doppler closest-point-of-approach, FDOA multi-pose grid, **Doppler-consistency geolocation**, kinematic synthetic-aperture DoA, phase-interferometry along track, ML grid fusion, EKF kinematic tracker. All in-process numpy / scipy. **Live capture has a hard GPS/INS dependency** — the receiver track *is* the aperture, so the recorder pairs each live fix (gpsd / serial NMEA incl. INS units / SDR GPSDO / browser geolocation) with the strongest peak near the carrier, and refuses to run from a manually-typed or stale position — or from a synthetic capture. **Fixes plot live on both maps** as the track refines — the position with its real CEP/error ellipse (1σ when the solver gives one, CEP50 circle otherwise, and an honest "range unobservable" point when the geometry is degenerate), and a line-of-bearing ray for the bearing-only methods.
  - **Doppler-consistency geolocation** locates a stationary emitter from a *single moving receiver* on an arbitrarily manoeuvring platform (sUAS / vehicle / man-pack — not just a straight constant-velocity pass like CPA). The measured peak offset is the vector Doppler plus an unknown constant LO offset; that offset is one physical constant, so it must be identical at every point along the track *only at the true emitter location*. The solver grid-searches (refining, halving each pass) for the position that minimises the variance of the implied offset — eliminating the unknown offset as a free mean. The reported CEP is scaled by the measurement noise, so a weakly-observable geometry (e.g. one straight leg) honestly reports a degenerate fix rather than a falsely tight one.
- **Per-identifier target tracking** — keyed by IMSI / TMSI / IMEI / RNTI / MAC / BLE / ICAO / DMR-RID / UAS serial / callsign. Peak-RSSI sampler, top-K, range estimate that auto-upgrades from Friis single-shot → multi-pose RSS-ML → AoA-fused ML grid as observations accumulate.
- **Network analysis** — hosts the passive monitors and groups every collected selector into the radio network it belongs to: subscribers under their serving cell (PLMN · LAC/TAC · CI), stations under their BSSID/SSID, radio IDs under their talk-group, drone serials under their Remote-ID operator, plus BLE / ADS-B / AIS. Three interchangeable views — an Excel-style spreadsheet (sortable, customizable base + live-discovered metadata columns), an analyst link chart clustering each network's hub with its members, and a collapsible network → members tree. Per-domain filter chips, search, Simple/Detailed density; preferences persist. Exports the filtered picture as CSV, a real `.xlsx` workbook (selectors + per-network summary), or an i2 Analyst's Notebook entities-and-links workbook ready for a spreadsheet Import Specification — all generated client-side, air-gap friendly.
...
- **Passive drone-detection watch mode (the Video tab)** — the man-portable-detector capability: a continuous sweep of the FPV/UAS bands that flags when a drone video/RF emitter appears, classifies it by **platform family** (DJI drone · analog FPV · digital FPV (HDZero/Walksnail) · ISR/COFDM downlink · military CDL · Remote-ID beacon) on top of the signal-type classifier, and tracks signal strength as a **proximity cue** with a rising/falling trend (rising = approaching). New detections raise an audible alarm + banner; operator RSSI/confidence thresholds and band scope the alerts; the live detection log shows first/last seen and an estimated range. Honest about a single antenna: a detection is presence + coarse range, so it plots at the **operator position** as a dashed proximity ring (never a bearing or a located fix) and pushes to CoT/ATAK as an unknown-air track with that range as the error radius. Locating the drone needs the DF array or its Remote-ID beacon.
...
**Mathematical verification suite** — the DF, geolocation, tracking and propagation maths are pinned by a synthetic-oracle pytest suite (172 tests) where *geometry is the oracle*: every test plants a known emitter / bearing / link, simulates the exact physics, and requires the estimator to invert it back — no golden files to drift. It checks not just point estimates but statistical consistency (the reported covariance ellipse actually contains the truth at its stated confidence), the CRLB, and closed-form agreement (FSPL / Hata / knife-edge / link-budget against their published formulas).

```bash
cd backend
.venv/bin/python -m pytest tests/ -q     # full suite incl. the math-verification modules
# or target one area:
.venv/bin/python -m pytest tests/test_math_df.py tests/test_math_geoloc.py \
                          tests/test_math_propagation.py tests/test_math_single_channel.py -q
```
