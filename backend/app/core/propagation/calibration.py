...
# Published reference anchors (route means). Weighted by sample count.
REFERENCE_ANCHORS: list[Measurement] = [
    Measurement(1800, 399.81, 142.42, 30, 1.5, "urban", 937,
                "Covenant U. route X (DOI 10.1016/j.dib.2018.02.026)"),
    Measurement(1800, 460.49, 139.72, 30, 1.5, "urban", 1229,
                "Covenant U. route Y (DOI 10.1016/j.dib.2018.02.026)"),
    Measurement(1800, 447.42, 146.34, 30, 1.5, "urban", 1450,
                "Covenant U. route Z (DOI 10.1016/j.dib.2018.02.026)"),
]
...
def calibrate(measurements: Optional[list[Measurement]] = None) -> dict:
    ...
    return {
        ...
        "note": ("Negative bias ⇒ the model under-predicts measured loss. The reference "
                 "anchors are route MEANS near one distance, so they pin each model's "
                 "offset/bias, not its distance slope — drop a full per-point CSV for that."),
        "sources": sorted({m.source for m in ms if m.source}),
    }
