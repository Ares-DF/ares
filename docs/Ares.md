...
**Performance / native acceleration.** The ITM port runs at ≈0.12 ms per point-to-point call (≈0.27 s for a 48×48 raster, ≈1.1 s for 96×96) — its two hottest helpers, `_dlthx` (the terrain interdecile range) and `_zlsq1` (the least-squares line fit), were Python loops over the profile and are now NumPy-vectorised, giving ≈1.5× over the previous ≈0.17 ms with **bit-identical output** (golden-pinned in the validation harness alongside the existing ITM reference pins). `_hzns` (the horizon walk) is left as a Python scan — it's sequential/stateful and a smaller share of the time. On a non-GPU box the per-pixel raster sweep (and multisite / Monte-Carlo) is the hot path, so `itm_its.py` carries a drop-in seam: if a compiled core — a Rust/PyO3, Cython, or Numba-jitted module exposing `itm_point_to_point(...) -> ITMResult` — is importable as `ares_rf_core`, it is used instead, with the pure-Python kept as the reference and the always-available fallback (`NATIVE_ITM_AVAILABLE` reports it; the harness pins keep the two in agreement). **Bit-validation:** `test_itm_validation` recomputes the named NTIA `itm.cpp` helper functions (`aknfe` knife-edge, `fht` smooth-earth height-gain, `h0f` forward-scatter frequency-gain, `ahd` scatter-distance, `qerfi` inverse-normal) and the FSL formula from the reference equations and asserts bit-equality (~1e-12; `qerfi` to the A&S-26.2.23 fit accuracy, ~5e-4); and if NTIA's `TestData` CSV is dropped at `tests/data/itm_reference.csv` it runs the *full* point-to-point port against every case and asserts |loss − expected| < 0.01 dB — so the closed-form pieces are bit-validated now, and the whole port is one CSV away from a full bit-for-bit check. Recommended ladder if profiling on real workloads shows ITM is the bottleneck: (1) NumPy-vectorise `_dlthx`/`_hzns` (free); (2) Numba `@njit` the inner loop (`pip install numba`, no build matrix, ~10–50×); (3) Cython or Rust only if those are not enough and this becomes a permanent heavily-used hot path — and even then scoped to a single crate (the build-matrix / FFI / dual-implementation cost is the price; the GPU path already covers the very large jobs).
...
**The ML signal classifier is now a concrete module, not just a hook.** `app/core/sdr/ml_signal_classifier.py` provides: `feature_vector(iq, fs)` — a fixed 14-element NumPy feature vector (occupied BW, spectral flatness, envelope CV, OFDM cyclic-prefix corr / FFT-length / guard fraction, the |C40/C42| cumulant ratio, symbol-rate, roll-off, FM line-rate ratio, and three spectral-correlation-function stats); `load_model(path)` (ONNX via `onnxruntime`, or `.pt`/`.pth` via `torch` — whichever is installed); a `Classifier` wrapper exposing the `classify(iq, fs) -> {feed_type, confidence, probs}` contract `uas_video.set_ml_classifier` expects; and `register(model_path)` to wire it in. The docstring documents how to train one (DroneRF / RadioML / weakly-labelled captures → a 1-D CNN on IQ, a 2-D CNN on the spectrogram/SCF, or trees on the feature vector → ONNX export → `register(...)`). The trained model is deployment-supplied — without `torch`/`onnxruntime` the feature extractor still runs and `register` reports what to install; the rule-based classifier remains the default. Status at `GET /uas/ml/status`.

**DJI DroneID v2 — a descrambler hook.** `remote_id.set_droneid_v2_descrambler(fn)` lets a deployment plug in the v2 de-obfuscation (the open `dji_droneid` tooling carries the AES-CTR key + nonce scheme — a fixed published descramble, not a comms decrypt; Ares does not vendor it). When registered, `parse_dji_droneid` de-obfuscates the tail before parsing (`v2_descrambled: true`); without one, the plaintext header is parsed and the note points at the hook. 9 more harness checks (`test_ml_classifier`). Routes: 111; harness: 108/108.
