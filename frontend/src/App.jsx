...
import { getTerrainProfile, getGpsFix } from './api/client'
...
const TX_COLORS = ['#00b4d8', '#f59e0b', '#06d6a0', '#ef4444', '#a78bfa', '#f97316']
...
// Read once at module load — used to hydrate useState initial values below
...
if (_s?.primaryTransmitter?.antenna) _s.primaryTransmitter.antenna = _migrateAntenna(_s.primaryTransmitter.antenna)
if (Array.isArray(_s?.extraTransmitters)) {
  _s.extraTransmitters = _s.extraTransmitters.map(t => t?.antenna ? { ...t, antenna: _migrateAntenna(t.antenna) } : t)
}
...
export default function App() {
  ...
  }, [lobs, lobGroups, capGroups, lobAlgorithm, sdrFeatures])
  ...
  const handleRemoveLoB = useCallback((id) => {
    setLobs(prev => prev.filter(l => l.id !== id))
  }, [])

  const handleUpdateLoB = useCallback((updated) => {
    setLobs(prev => prev.map(l => l.id === updated.id ? updated : l))
  }, [])
